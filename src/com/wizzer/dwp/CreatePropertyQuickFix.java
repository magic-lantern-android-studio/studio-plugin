...
public class CreatePropertyQuickFix extends BaseIntentionAction {
            ...
            public void run() {
                ...
                if (virtualFiles.size() == 1) {
                    ...
                    final FileChooserDescriptor descriptor =
                            FileChooserDescriptorFactory.createSingleFileDescriptor(DigitalWorkprintFileType.INSTANCE);
    ...
    private void createProperty(final Project project, final VirtualFile file) {
        new WriteCommandAction.Simple(project) {
            @Override
            public void run() {
                DigitalWorkprintFile simpleFile = (DigitalWorkprintFile) PsiManager.getInstance(project).findFile(file);
                ...
                if (lastChildNode != null/* && !lastChildNode.getElementType().equals(SimpleTypes.CRLF)*/) {
                    simpleFile.getNode().addChild(DigitalWorkprintElementFactory.createCRLF(project).getNode());
                }
                // IMPORTANT: change spaces to escaped spaces or the new node will only have the first word for the key
                DwpProperty property = DigitalWorkprintElementFactory.createProperty(project, key.replaceAll(" ", "\\\\ "), "");
